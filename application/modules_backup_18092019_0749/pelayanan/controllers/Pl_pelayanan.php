...
class Pl_pelayanan extends MX_Controller {
    ...
    public function tindakan($id='', $no_kunjungan='')
    {
        ...
        $this->breadcrumbs->push('Add '.strtolower($this->title).'', 'Pl_pelayanan/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
        /*get value by id*/
        $data['value'] = $this->Pl_pelayanan->get_by_id($id); //echo '<pre>'; print_r($this->db->last_query());die;
        /*mr*/
        /*type*/
        if($data['value']->flag_ri==1){
            $kode_klas = $data['value']->kelas_ri;
    ...
    public function get_transaksi_by_id()
    {
        ...
        $html_tag .= '<th style="text-align:right;width:120px">Biaya</th>';
        $html_tag .= '<th style="text-align:right;width:120px">Persentase (%)</th>';
        ...
        foreach ($komponen as $key => $value) {
            ...
            if($vals != '0'){
                ...
                $html_tag .= '<td align="right"><input type="text" value="'.(int)$data->$key.'" name="'.$key.'_'.$id.'" id="'.$key.'_'.$id.'" style="text-align:right;width:100px !important"></td>';
                $html_tag .= '<td align="right"><input type="text" onchange="changeDiscount('."'".$key."'".','.$id.')" style="text-align:center;margin-bottom:5px;width:70px" value="0" id="diskon_'.$key.'_'.$id.'"></td>';
