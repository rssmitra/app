...
class Ws_index extends MX_Controller {
    ...
    function prosesInsertSep($val){
        /*insert sep*/
            ...
            $response = json_encode(array('status' => $result->metaData->code, 'message' => $result->metaData->message, 'result' => $sep, 'redirect' => base_url().'ws_bpjs/ws_index?modWs=InsertSep' ));
    ...
    public function postDataRuanganRs(){
        ...
        foreach ($list->result() as $key => $value) {
            $getData[$value->kode_bagian][$value->kode_klas_bpjs] = array(
                ...
                'tersedia' => $this->countRuangan($value->kode_bagian, $value->kode_klas_bpjs),
                'tersediapriawanita' => 0,
                'namaruang' => $value->nama_bagian,
                'kodeklas' => $value->kode_klas_bpjs,
                'namaklas' => $value->nama_klas_bpjs,
                'tersediapria' => 0,
                'kapasitas' => 0,
                'tersediawanita' => 0,
                );
