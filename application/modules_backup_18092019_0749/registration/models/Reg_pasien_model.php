...
class Reg_pasien_model extends CI_Model {
	...
	
	{
		...
		$this->db->where( 'regon_booking_no_mr', $mr );

		$this->db->order_by( 'regon_booking_id', 'DESC' );
	...
	
	{
		...
		/*data transaksi*/
		$transaksi = $this->db->select('kode_trans_pelayanan, no_kunjungan, nama_tindakan, mt_jenis_tindakan.jenis_tindakan, kode_jenis_tindakan, tgl_transaksi, kode_tc_trans_kasir, nama_pegawai')->join('mt_jenis_tindakan','mt_jenis_tindakan.kode_jenis_tindakan=tc_trans_pelayanan.jenis_tindakan','left')->join('mt_karyawan','mt_karyawan.kode_dokter=tc_trans_pelayanan.kode_dokter1','left')->get_where('tc_trans_pelayanan', array('no_registrasi' => $no_registrasi) )->result();
	...
	public function delete_kunjungan($no_kunjungan){
		...
		$this->db->where(" id_ak_tc_transaksi in (select id_ak_tc_transaksi from ak_tc_transaksi 
		where kode_tc_trans_kasir in (select kode_tc_trans_kasir from tc_trans_pelayanan where no_kunjungan=".$no_reg."))")->delete("ak_tc_transaksi_det");
	...
	public function delete_registrasi_and_kunjungan($no_reg, $no_kunjungan){
		...
		/*find data transaksi kasir*/
		$trans_kasir = $this->db->query("select * from tc_trans_kasir 
		where kode_tc_trans_kasir in (select kode_tc_trans_kasir from tc_trans_pelayanan where no_registrasi=".$no_reg.")");
