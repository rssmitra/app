...
class Reg_pm_model extends CI_Model {
	...
	public function get_hasil_pm($no_registrasi, $no_kunjungan, $kode_bagian='', $flag_mcu='' ){
		...
		if($flag_mcu==1){
			...
			where a.kode_mt_hasilpm=b.kode_mt_hasilpm AND a.kode_trans_pelayanan in (
			select kode_trans_pelayanan from tc_trans_pelayanan where no_kunjungan='.$no_kunjungan.') AND b.kode_bagian='."'".$kode_bagian."'".'
			order by b.nama_pemeriksaan ASC';
		...
		foreach ($getDataAll as $key_title => $val_title) { $no++;
			...
			$html .= '</tr>';
			foreach ($val_title['detail'] as $key => $value) {
				$detail_item = ($value->detail_item_1!='' || $value->detail_item_1!= NULL)?'('.$value->detail_item_1.')':'';
				$html .= '<tr>';
				$html .= '<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$value->nama_pemeriksaan.' '.$detail_item.'</td>';
				$html .= '<td align="center">'.$value->hasil.'</td>';
				$html .= '<td align="center">'.$value->standar_hasil_pria.'</td>';
