...
class Entry_resep_ri_rj_model extends CI_Model {
	...
	var $select = 'kode_bagian, kode_bagian_asal, tgl_pesan, status_tebus, jumlah_r, lokasi_tebus, keterangan, no_registrasi, no_kunjungan, fr_listpesanan_v.kode_perusahaan, kode_klas, fr_listpesanan_v.kode_kelompok, nama_pegawai, nama_lokasi, nama_bagian, kode_dokter, kode_pesan_resep, no_mr, nama_pasien, nama_perusahaan, nama_kelompok';

	var $order = array('tgl_pesan' => 'ASC', 'kode_pesan_resep' => 'ASC');
	...
	private function _get_datatables_query()
	{
		...
		if($_GET['flag']=='RJ'){
			$this->db->where('(kode_bagian_asal like '."'02%'".' OR kode_bagian_asal like '."'01%'".')');
		}else{
			$this->db->where('(kode_bagian_asal like '."'03%'".')');
		}
