...
class Process_entry_resep_model extends CI_Model {
	...
	public function format_no_resep($flag)
	{
		
		/*get max total resep by day*/
		$this->db->where('YEAR(tgl_trans)='.date('Y').' AND MONTH(tgl_trans)='.date('m').' AND DAY(tgl_trans)='.date('d').'');
		$max = $this->db->get('fr_tc_far')->num_rows();
		/*plus 1*/
		$max_row = $max + 1;
		/*format*/
		$format = $flag.'/'.date('my').'00'.$max_row;
