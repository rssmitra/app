<script type="text/javascript">
  ...
  $(document).ready(function(){
    ...
    oTable = $('#dt_selected_brg').DataTable({ 
      ...
      "ajax": {
          "url": "purchasing/permintaan/Req_selected_detail_brg_unit/get_data?flag="+$('#flag_string').val()+"&key="+$('#keyword').val()+"&search_by="+$('#search_by').val()+"",
          "type": "POST"
      },

      "columnDefs": [
          { "aTargets" : [ 9 ], "mData" : 9, "sClass":  "btn-add-to-cart"}, 
    ...
    $('#dt_selected_brg tbody').on( 'click', 'td.btn-add-to-cart', function (e) {
        ...
        // here process
        var post_data = {
          id_tc_permintaan_inst : $('#id_tc_permintaan_inst').val(),
          kode_brg : kode_brg,
          jumlah_permintaan : value_input,
          satuan : satuan,
          flag : $('#flag_string').val(),
        }

        $.ajax({
          url: 'purchasing/pendistribusian/Distribusi_permintaan/process_add_brg',
          type: "post",
          data: post_data,
          dataType: "json",
          beforeSend: function() {
