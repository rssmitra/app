...
class Po_revisi_model extends CI_Model {
	...
	public function rollback_po($flag, $id)
	{
		...
		$this->db->where('id_tc_permohonan IN (SELECT id_tc_permohonan FROM '.$tc_po.'_det WHERE id_tc_po='.$id.' GROUP BY id_tc_permohonan)')->update($tc_permohonan.'_det', array('status_po' => NULL) );
