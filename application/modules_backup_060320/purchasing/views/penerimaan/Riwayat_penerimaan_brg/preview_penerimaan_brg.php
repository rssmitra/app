  ...
  <table class="table-utama" style="width: 100% !important;margin-top: 10px; margin-bottom: 10px">
    <thead>
        <tr style="background-color: #FFF;color: #0a0a0a;font-weight: bold; border: 1px solid black; border-collapse: collapse">
          ...
          <td rowspan="2" style="text-align:center; width: 60px; border: 1px solid black; border-collapse: collapse">Jumlah<br>Kirim</td>
    ...
    <tbody>
        ...
        <?php 
          $count = count($penerimaan_data);
          $no=0; 
          foreach($penerimaan_data as $key_dt=>$row_dt) : $no++; 
          if(count($row_dt) > 0){
            foreach($row_dt as $row_sub_data){
              $jumlah_pesan[$key_dt][] = $row_sub_data->jumlah_besar;
              $jumlah_kirim[$key_dt][] = $row_sub_data->jumlah_kirim;
              $jumlah_harga_netto[$key_dt][] = $row_sub_data->harga_satuan;
            }
          }else{
            $jumlah_pesan[$key_dt][] = $row_dt[0]->jumlah_besar;
            $jumlah_kirim[$key_dt][] = $row_dt[0]->jumlah_kirim;
            $jumlah_harga_netto[$key_dt][] = $row_dt[0]->harga_satuan;
          }
          
          // $total_harga = array_sum($jumlah_harga_netto[$row_dt[0]->kode_brg]) - $row_dt[0]->discount_rp;

          $arr_total_harga[] = $row_dt[0]->total_harga;
            
        ?>
            <tr>
              <td style="text-align:center; border-left: 1px solid black; border-collapse: collapse"><?php echo $no?></td>
              <td style="border-left: 1px solid black; border-collapse: collapse"><?php echo $row_dt[0]->kode_brg.' - '.$row_dt[0]->nama_brg?></td>
              <td style="text-align:center; border-left: 1px solid black; border-collapse: collapse"><?php echo $row_dt[0]->content?></td>
              <td style="text-align:center; border-left: 1px solid black; border-collapse: collapse"><?php echo $row_dt[0]->satuan_besar?></td>
              <td style="text-align:center; border-left: 1px solid black; border-collapse: collapse"><?php echo array_sum($jumlah_pesan[$row_dt[0]->kode_brg])?></td>
              <td style="text-align:center; border-left: 1px solid black; border-collapse: collapse"><?php echo array_sum($jumlah_kirim[$row_dt[0]->kode_brg])?></td>
              <td style="text-align:right; border-left: 1px solid black; border-collapse: collapse"><?php echo number_format($row_dt[0]->harga_satuan).',-'; ?></td>
              <td style="text-align:center; border-left: 1px solid black; border-collapse: collapse"><?php echo $row_dt[0]->discount; ?></td>
              <td style="text-align:right; border-left: 1px solid black; border-collapse: collapse"><?php echo number_format($row_dt[0]->discount_rp).',-'; ?></td>
              <td style="text-align:right; border-left: 1px solid black; border-collapse: collapse"><?php echo number_format($row_dt[0]->total_harga).',-';?></td>
            </tr>
            <?php endforeach;?>
            <?php 
              for ($i=$count; $i < 15; $i++) { 
                echo '<tr>';
                echo '<td style="text-align:right; border-left: 1px solid black; border-collapse: collapse">&nbsp;</td>';
                echo '<td style="text-align:right; border-left: 1px solid black; border-collapse: collapse">&nbsp;</td>';
                echo '<td style="text-align:right; border-left: 1px solid black; border-collapse: collapse">&nbsp;</td>';
                echo '<td style="text-align:right; border-left: 1px solid black; border-collapse: collapse">&nbsp;</td>';
                echo '<td style="text-align:right; border-left: 1px solid black; border-collapse: collapse">&nbsp;</td>';
            ...
            ?>
            <tr>
              <td colspan="6" style="text-align:left; border-top: 1px solid black; border-collapse: collapse">Terbilang : <br>
              <b><i>"<?php $terbilang = new Kuitansi(); echo ucwords($terbilang->terbilang(array_sum($arr_total_harga)))?> Rupiah"</i></b>
              </td>
              <td colspan="3" style="text-align:right; padding-right: 20px; border-top: 1px solid black; border-collapse: collapse">Total Barang Diterima </td>
              <td style="text-align:right; border-top: 1px solid black; border-collapse: collapse; font-size: 14px; font-weight: bold"><?php echo number_format(array_sum($arr_total_harga))?></td>
            </tr>
            <!-- <tr>
              <td colspan="9" style="text-align:right; padding-right: 20px; border-top: 0px solid black; border-collapse: collapse">PPN </td>
              <td style="text-align:right; border: 0px solid black; border-collapse: collapse"><?php echo number_format($penerimaan->ppn)?></td>
            </tr>

            <tr>
              <td colspan="9" style="text-align:right; padding-right: 20px; border-top: 0px solid black; border-collapse: collapse">Total </td>
              <td style="text-align:right; border: 0px solid black; border-collapse: collapse"><?php echo number_format($penerimaan->total_stl_ppn)?></td>
            </tr> -->
            <!-- <tr>
