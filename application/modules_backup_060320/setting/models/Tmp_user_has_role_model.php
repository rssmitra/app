...
class Tmp_user_has_role_model extends CI_Model {
	...
	var $select = 'tmp_user.email, fullname, username, password, is_active, is_deleted, user_id, tmp_user.updated_date, tmp_user.updated_by, tmp_user.created_by, tmp_user.created_date';
	...
	private function _main_query(){
		...
		$this->db->select("CAST((
				          ...
				          FROM dbo.tmp_user_has_role T
				          LEFT JOIN tmp_mst_role ON tmp_mst_role.role_id=T.role_id
				          WHERE tmp_user.user_id = T.user_id 
				          FOR XML PATH(''))as varchar(max)) as role_name ");
