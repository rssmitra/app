...
class Penerimaan_brg_model extends CI_Model {
	...
	private function _main_query(){
		...
		$t_penerimaan = ($_GET['flag']=='non_medis')?'tc_penerimaan_barang_nm':'tc_penerimaan_barang';
		$backmonth = date('m') - 3;
		...
		// 		LEFT JOIN '.$table.'_det y on y.id_tc_po_det=x.id_tc_po_det
		// 		LEFT JOIN '.$table.' z on z.id_tc_po=y.id_tc_po
		// 		GROUP BY z.id_tc_po, x.id_tc_po_det, y.jumlah_besar_acc
		// 		HAVING y.jumlah_besar_acc > SUM(jumlah_kirim)
		// )');
	...
	public function get_penerimaan_brg($flag, $id){
		...
		$this->db->select('y.kode_penerimaan, y.tgl_penerimaan, z.no_po, a.id_tc_po, a.id_tc_po_det, a.kode_brg, a.content, c.nama_brg, c.satuan_besar, c.satuan_kecil, jumlah_besar, SUM(b.jumlah_kirim) as jumlah_kirim, CAST(a.harga_satuan_netto as INT) as harga_satuan_netto, CAST(a.jumlah_harga_netto as INT) as total_harga_netto, CAST(a.harga_satuan as INT) as harga_satuan, CAST(a.jumlah_harga as INT) as total_harga, batch_log.jml_diterima, batch_log.kode_box, batch_log.kode_pcs, batch_log.no_batch, batch_log.jenis_satuan, batch_log.tgl_expired, batch_log.is_expired, batch_log.id_tc_batch_log, a.discount,a.discount_rp, z.ppn, p.namasupplier, p.alamat, p.telpon1, z.total_sbl_ppn, z.total_stl_ppn, z.discount_harga, y.petugas, y.dikirim, b.jumlah_kirim_decimal');
		...
		$this->db->group_by('y.kode_penerimaan, y.tgl_penerimaan, z.no_po, a.id_tc_po, a.id_tc_po_det, a.kode_brg, a.content, c.nama_brg, jumlah_besar, c.satuan_besar, c.satuan_kecil, CAST(a.harga_satuan_netto as INT), CAST(a.jumlah_harga_netto as INT), CAST(a.harga_satuan as INT), CAST(a.jumlah_harga as INT), batch_log.jml_diterima, batch_log.kode_box, batch_log.kode_pcs, batch_log.no_batch, batch_log.jenis_satuan, batch_log.tgl_expired, batch_log.is_expired, batch_log.id_tc_batch_log, a.discount, z.ppn, p.namasupplier, p.alamat, p.telpon1, z.total_sbl_ppn, z.total_stl_ppn, z.discount_harga, a.discount_rp, y.petugas, y.dikirim, b.jumlah_kirim_decimal');
		$this->db->order_by('c.nama_brg ASC');
		return $this->db->get()->result();
	}
