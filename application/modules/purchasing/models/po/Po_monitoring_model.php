...
class Po_monitoring_model extends CI_Model {

	var $table_nm = 'tc_po_nm_det';
	var $table = 'tc_po_det';
	var $column = array('d.no_po','c.namasupplier');
	var $select = 'd.id_tc_po, d.no_po, d.tgl_po, d.ppn, d.total_sbl_ppn, d.total_stl_ppn, d.discount_harga, d.term_of_pay, b.username, d.diajukan_oleh, d.disetujui_oleh, c.namasupplier, d.jenis_po, e.nama_brg, a.kode_brg, e.satuan_besar, a.content';
	...
	private function _main_query(){
		...
		$this->db->select('a.jumlah_besar_acc as jumlah_besar');
		...
		$this->db->join(''.$mt_barang.' e','e.kode_brg=a.kode_brg', 'left');
		$this->db->join('dd_user b','b.id_dd_user=d.user_id', 'left');
		$this->db->join('mt_supplier c','c.kodesupplier=d.kodesupplier', 'left');
	}
	...
	private function _get_datatables_query()
	{
		...
		if( isset( $_GET['search_by'] )){
			if( $_GET['search_by'] == 'month' ){
				$this->db->where('MONTH(d.tgl_po) = ', $_GET['month']);
				$this->db->where('YEAR(d.tgl_po) = ', $_GET['year']);
			}elseif ( $_GET['search_by'] == 'tgl_po' ) {
				if (isset($_GET['from_tgl']) AND $_GET['from_tgl'] != '' || isset($_GET['to_tgl']) AND $_GET['to_tgl'] != '') {
					$this->db->where("convert(varchar,d.tgl_po,23) between '".$_GET['from_tgl']."' and '".$_GET['to_tgl']."'");
				}else{
					$this->db->where('MONTH(d.tgl_po) = ', date('m'));
					$this->db->where('YEAR(d.tgl_po) = ', date('Y'));
				}
			}elseif ( $_GET['search_by'] == 'supplier' ) {
				$this->db->where('d.kodesupplier', $_GET['kodesupplier']);
				$this->db->where('YEAR(d.tgl_po) = ', $_GET['year']);
			}elseif ( $_GET['search_by'] == 'no_po' ) {
				$this->db->like( 'd.no_po', $_GET['keyword'] );
				if( isset($_GET['year_nopo']) && $_GET['year_nopo'] != '' ){
					$this->db->where('YEAR(d.tgl_po) = ', $_GET['year_nopo']);
				}
			}else{
				$this->db->where('MONTH(d.tgl_po) = ', date('m'));
				$this->db->where('YEAR(d.tgl_po) = ', date('Y'));
		...
		$this->db->where('a.jumlah_besar > 0 ');
	...
	public function get_po($flag, $id){
		...
		$this->db->join($tc_penerimaan.' f', 'f.id_tc_po_det=a.id_tc_po_det', 'left');
	...
	public function delete_po($flag, $id)
	{
		$tc_po = ($flag=='non_medis')?'tc_po_nm':'tc_po';
