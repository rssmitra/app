...
class Import_hasil_verif_bpjs_model extends CI_Model {
    ...
    public function update_detail_tarif_ori($table, $data){
        ...
        foreach($data as $k=>$value){
            /*loop detail tarif*/
            //foreach ($detail_tarif as $key => $value) {
                /*cek terlebih dahulu apakah sudah ada data nya atau tidak*/
                $is_exist = $this->db->get_where($table, array('kode_tarif' => $value['kode_tarif'], 'kode_klas' => $value['kode_klas']) )->num_rows();

                if($is_exist > 0){

                    if($this->db->update($table, $value, array('kode_tarif' => $value['kode_tarif'], 'kode_klas' => $value['kode_klas']))){
                        echo nl2br($value['kode_tarif'].' => '.$value['bill_rs'].' | '.$value['bill_dr1'].' | '.$value['bill_dr2'].' | '.$value['bhp'].' | '.$value['pendapatan_rs'].' row updated successfull').PHP_EOL.'<br>'; 
                    }else{
                        echo nl2br($value['kode_tarif'].' row updated failed').PHP_EOL.'<br>';
                    }
    ...
    public function update_tarif($table, $data){
        ...
        $is_exist = $this->db->where('kode_bagian', $data['kode_bagian'])
                             ->where('nama_tarif', trim($data['nama_tarif']))
                             ->from($table)->order_by('revisi_ke', 'DESC')->get();
        //echo '<pre>'; print_r($is_exist);die;
        if($is_exist->num_rows() > 0){
            /*add field revisi*/
            $row_data = $is_exist->row();
            $revisi = $row_data->revisi_ke + 1;
            $field_revisi = array('kode_tarif' => $row_data->kode_tarif, 'revisi_ke' => $revisi, 'kode_tindakan' => $row_data->kode_tindakan, 'jenis_tindakan' => $row_data->jenis_tindakan);
            $merge_array = array_merge($data, $field_revisi);
            //echo '<pre>'; print_r($merge_array);die;
            /*update data*/
