...
class Mst_religion extends MX_Controller {
    ...
    public function form($id='')
    {
        ...
        if( $id != '' ){
            ...
            $this->breadcrumbs->push('Edit '.strtolower($this->title).'', 'reference/identity/mst_religion/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
            /*get value by id*/
            ...
            $this->breadcrumbs->push('Add '.strtolower($this->title).'', 'reference/identity/mst_religion/'.strtolower(get_class($this)).'/form');
    ...
    public function show($id)
    {
        ...
        $this->breadcrumbs->push('View '.strtolower($this->title).'', 'reference/identity/mst_religion/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
    ...
    public function process()
    {
        ...
        {                       
            ...
            if($id==0){
                ...
                /*save post data*/
                $this->mst_religion->save($dataexc);
                $newId = $this->db->insert_id();
                /*save logs*/
                $this->logs->save('mst_religion', $newId, 'insert new record on '.$this->title.' module', json_encode($dataexc),'religion_id');
            }else{
                $dataexc['updated_date'] = date('Y-m-d H:i:s');
                $dataexc['updated_by'] = json_encode(array('user_id' =>$this->regex->_genRegex($this->session->userdata('user')->user_id,'RGXINT'), 'fullname' => $this->regex->_genRegex($this->session->userdata('user')->fullname,'RGXQSL')));
                /*update record*/
                $this->mst_religion->update(array('religion_id' => $id), $dataexc);
                $newId = $id;
                /*save logs*/
                $this->logs->save('mst_religion', $newId, 'update record on '.$this->title.' module', json_encode($dataexc),'religion_id');
            }
    ...
    public function delete()
    {
        ...
        if($id!=null){
            if($this->mst_religion->delete_by_id($toArray)){
                $this->logs->save('mst_religion', $id, 'delete record', '', 'religion_id');
