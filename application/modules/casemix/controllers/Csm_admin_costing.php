...
class Csm_admin_costing extends MX_Controller {
    ...
    function __construct() {
        ...
        parent::__construct();
        /*breadcrumb default*/
        $this->breadcrumbs->push('Index', 'casemix/Csm_admin_costing');
        /*session redirect login if not login*/
        if($this->session->userdata('logged')!=TRUE){
            redirect(base_url().'login');exit;
        }
        /*load model*/
        $this->load->model('Csm_admin_costing_model', 'Csm_admin_costing');
    ...
    public function index() { 
        ...
        /*load view index*/
        $this->load->view('Csm_admin_costing/index', $data);
    }
    ...
    public function editBilling($no_registrasi, $tipe)
    {
        ...
        $this->breadcrumbs->push('Edit function', 'Csm_admin_costing/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$no_registrasi);
       ...
       //echo '<pre>';print_r($dataBilling);die;
        ...
        //echo '<pre>';print_r($data);die;
        $this->load->view('Csm_admin_costing/'.$view_name.'', $data);
    }
    ...
    public function getDetail($no_registrasi, $tipe){
        ...
        $data = json_decode($this->Csm_admin_costing->getDetailData($no_registrasi));
        
        /*cek apakah data sudah pernah diinsert ke database atau blm*/
        if( $this->Csm_admin_costing->checkExistingData($no_registrasi) ){
            /*no action if data exist, continue to view data*/
        }else{
        /*jika data belum ada atau belum pernah diinsert, maka insert ke table*/
            /*insert data untuk pertama kali*/
            if( $data->group && $data->kasir_data && $data->trans_data )
            $this->Csm_admin_costing->insertDataFirstTime($data, $no_registrasi);
        }
        //print_r($data);die;
        if($tipe=='RJ'){
            $html = $this->Csm_admin_costing->getDetailBillingRJ($no_registrasi, $tipe, $data);
        }else{
            $html = $this->Csm_admin_costing->getDetailBillingRI($no_registrasi, $tipe, $data);
        }
    ...
    {   
        ...
        if( $this->Csm_admin_costing->update_status_nk_kode_perusahaan($no_registrasi) ){
            /*get data trans pelayanan by no registrasi from sirs*/
            $sirs_data = json_decode($this->Csm_billing_pasien->getDetailData($no_registrasi));
            $this->Csm_billing_pasien->insertDataFirstTime($sirs_data, $no_registrasi);
    ...
    {   
        $no_registrasi = $this->input->post('no_registrasi');
        if( $this->Csm_admin_costing->submit_kasir($no_registrasi) ){
    ...
    public function submit_multiple()
    {
        ...
        if ( isset($_POST['ID']) ) {
            ...
            foreach ($arr_id as $key => $value) {
                ...
                $this->Migration->update_status_nk_kode_perusahaan($value);
                /*get data trans pelayanan by no registrasi from sirs*/
                $sirs_data = json_decode($this->Csm_billing_pasien->getDetailData($value));

                if( $this->Csm_billing_pasien->checkExistingData($value) ){
                /*no action if data exist, continue to view data*/
                }else{
                /*jika data belum ada atau belum pernah diinsert, maka insert ke table*/
                    /*insert data untuk pertama kali*/
                    if( $sirs_data->group && $sirs_data->kasir_data && $sirs_data->trans_data )
                    $this->Csm_billing_pasien->insertDataFirstTime($sirs_data, $value);
                }
                ...
                /*update to sirs*/
                $data_update_sirs = array(
                                        'csm_rp_no_sep' => $original->csm_rp_no_sep,
                                        'csm_rp_tgl_masuk' => $this->tanggal->formatDateTimeSql($original->csm_rp_tgl_masuk),
                                        'csm_rp_tgl_keluar' => $this->tanggal->formatDateTimeSql($original->csm_rp_tgl_masuk),
                                    );
                $this->Csm_billing_pasien->updateSirs($value,  $data_update_sirs);
                
                /*csm_reg_pasien*/
                $update_csm_reg_pasien = array(
                    'is_submitted' => $this->regex->_genRegex('Y', 'RGXAZ'),
                    'updated_date' => date('Y-m-d H:i:s'),
                    'updated_by' => $this->regex->_genRegex($this->session->userdata('user')->fullname,'RGXQSL'),
                );
                $exc_qry = $this->db->update('csm_reg_pasien', $update_csm_reg_pasien, array('no_registrasi' => $value));

                $this->logs->save('csm_reg_pasien', $value, 'update record', json_encode($update_csm_reg_pasien));
