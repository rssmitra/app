...
class Adm_blm_submit extends MX_Controller {
    ...
    function __construct() {
        ...
        parent::__construct();
        /*breadcrumb default*/
        $this->breadcrumbs->push('Index', 'adm_pasien/loket_kasir/Adm_blm_submit');
        /*session redirect login if not login*/
        if($this->session->userdata('logged')!=TRUE){
            echo 'Session Expired !'; exit;
        }
        /*load model*/
        $this->load->model('adm_pasien/loket_kasir/Adm_blm_submit_model', 'Adm_blm_submit');
    ...
    public function index() { 
        ...
        /*show datatables*/
        $data['dataTables'] = $this->load->view('loket_kasir/Adm_blm_submit/temp_trans_pasien', $data, true);
        /*load view index*/
        $this->load->view('loket_kasir/Adm_blm_submit/index', $data);
    }
    ...
    {
        /*get data from model*/
        $list = $this->Adm_blm_submit->get_datatables();
        // print_r($list);die;
        $data = array();
        $arr_total = array();
        $no = $_POST['start'];
        foreach ($list as $row_list) {
            ...
            if( $_GET['pelayanan'] != 'RI' ){
                if( substr($row_list[0]['kode_bagian_masuk'], 0, 2) != '03'){
                    ...
                    $row[] = $row_list[0]['no_mr'].'<br>'.$row_list[0]['nama_pasien'];
                    $row[] = ucwords($row_list[0]['nama_bagian']);
                    $row[] = ($row_list[0]['nama_perusahaan'])?$row_list[0]['nama_perusahaan']:'UMUM';
                    $row[] = str_replace('/', '<br>', ucwords(str_replace('-','',$row_list[0]['no_telp'])).'/'.$row_list[0]['no_hp']);
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Adm_blm_submit->count_all(),
                        "recordsFiltered" => $this->Adm_blm_submit->count_filtered(),
                        "data" => $data,
    ...
    {
        /*get data from model*/
        $list = $this->Adm_blm_submit->get_total_billing(); 
    ...
    public function getDetailTransaksi($no_registrasi){
        ...
        $akunting = $this->Adm_blm_submit->get_jurnal_akunting($no_registrasi);
        $data = array(
            'result' => $result,
        ...
        $html = $this->load->view('loket_kasir/Adm_blm_submit/detail_transaksi_view', $data, true);
