...
class References extends MX_Controller {
  ...
  //   	$html .= '</table>';
	...
	public function get_riwayat_pm($mr, $return = ''){
		...
		$result = $this->db->join('mt_bagian', 'mt_bagian.kode_bagian=th_riwayat_pasien.kode_bagian','left')->order_by('no_kunjungan','DESC')->where('DATEDIFF(year,tgl_periksa,GETDATE()) < 2 ')->get_where('th_riwayat_pasien', array('no_mr' => $no_mr))->result();
