              ...
              <form class="form-horizontal" method="post" id="form-default" action="<?php echo site_url('inventory/master/Inv_master_barang/process?flag='.$flag_string.'')?>" enctype="multipart/form-data">
                ...
                <div>
                  ...
                  <table class="table">
                    ...
                    <tbody>
                      <?php $no = 0; foreach ($history_po->result() as $key => $row) : $no++; ?>
                      <tr>
                        <td><?php echo $no; ?></td>
                        <td><?php echo $row->no_po; ?></td>
                        <td align="center"><?php echo $this->tanggal->formatDateDmy($row->tgl_po); ?></td>
                        <td><?php echo $row->namasupplier.'<br><small>'.$row->alamat.'<br>Telp. '.$row->telpon1.'</small>'; ?></td>
                        <td align="right"><?php echo number_format($row->jumlah_besar_acc, 2); ?></td>
                        <td align="right"><?php echo number_format($row->harga_satuan, 2); ?></td>
                        <td align="center"><?php echo number_format($row->discount, 2); ?></td>
                        <td align="right"><?php echo number_format($row->jumlah_harga, 2); ?></td>
                        <td align="center"><?php echo $this->tanggal->formatDateDmy($row->tgl_kirim).', '.$row->petugas; ?></td>
                      </tr>
                      <?php endforeach; ?>
                    </tbody>
                  </table>
