...
$(document).ready(function() {
  //initiate dataTables plugin
    oTable = $('#table-pesan-resep').DataTable({ 
      ...
      "ajax": {
          "url": "farmasi/Resep_ri/get_data_by_id?q="+<?php echo $value->no_kunjungan ?>,
          "type": "POST"
      },
      "columnDefs": [
        { 
    ...
    $('#table-pesan-resep tbody').on('click', 'td.details-control', function () {
            ...
            if ( row.child.isShown() ) {
                ...
                $.getJSON("farmasi/Resep_ri/getDetail/" + kode_pesan_resep +"?no_mr=<?php echo $value->no_mr?>", '', function (data) {
                    response_data = data;
                    // Open this row
...
function proses_retur_obat(kode_trans_far){
    ...
    if (answer){
        ...
        $.ajax({
            ...
            type: "post",
            data: $('#form_retur_'+kode_trans_far+'').serialize(),
            dataType: "json",
            beforeSend: function() {
                achtungShowLoader();  
            },
      ...
      $.ajax({
          ...
          complete: function(xhr) {     
            ...
            if(jsonResponse.status === 200){
              ...
              $('#page-area-content').load('farmasi/Entry_resep_ri_rj/form/'+kode_pesan_resep+'?mr='+$('#no_mr').val()+'&tipe_layanan=RI');
            }else{
              $.achtung({message: jsonResponse.message, timeout:5});
            }
            achtungHideLoader();
