...
<center><span><strong><u>NOTA RETUR FARMASI</u></strong><br>
No. RTR - <?php echo strtoupper($retur_data[0]->no_retur)?> / <?php echo $retur_data[0]->kode_trans_far?>
</span></center>
<span style="position: absolute; margin-left:63%;transform: rotate(0deg) !important;" class="stamp is-draft">Nota Retur</span>
<table>
  <tr style="border: 1px solid black; border-collapse: collapse">
    <td width="100px">Tanggal</td>
    <td style="background-color: #FFF;color: #0a0a0a;border: 1px solid #FFF; border-collapse: collapse"> : <?php echo $this->tanggal->formatDateTime($retur_data[0]->tgl_his_retur) ?></td>
  </tr>
  <tr style="border: 1px solid black; border-collapse: collapse">
    <td width="100px">Nama Pasien</td>
    <td style="background-color: #FFF;color: #0a0a0a;border: 1px solid #FFF; border-collapse: collapse"> : <?php echo ucwords($retur_data[0]->nama_pasien)?></td>
  </tr>
  <tr style="border: 1px solid black; border-collapse: collapse">
    <td width="100px">No. MR</td>
    <td style="background-color: #FFF;color: #0a0a0a;border: 1px solid #FFF; border-collapse: collapse"> : <?php echo $retur_data[0]->no_mr?></td>
  </tr>
  <tr style="border: 1px solid black; border-collapse: collapse">
    <td width="100px">Dokter</td>
    <td style="background-color: #FFF;color: #0a0a0a;border: 1px solid #FFF; border-collapse: collapse"> : <?php echo ucwords($retur_data[0]->dokter_pengirim)?></td>
  </tr>
  <tr style="border: 1px solid black; border-collapse: collapse">
    <td width="100px">Asal</td>
    <td style="background-color: #FFF;color: #0a0a0a;border: 1px solid #FFF; border-collapse: collapse"> : <?php echo $retur_data[0]->nama_bagian?></td>
  </tr>
</table>
...
<table class="table-utama" style="width: 100% !important;margin-top: 10px; margin-bottom: 10px">
  <thead>
      ...
      <?php 
        $no=0; 
        $arr_total = [];
        foreach($retur_data as $key_dt=>$row_dt) : $no++; 
        
          $harga_jual = $row_dt->harga_jual_satuan;
          $jumlah_obat = $row_dt->jumlah_retur_his;
          $subtotal = ($harga_jual * $jumlah_obat) + $row_dt->jasa_r; 
          $arr_total[] = $subtotal;
          $desc = $row_dt->nama_brg;
          $satuan = $row_dt->satuan_kecil;
      ?>
...
<table style="width: 100% !important; text-align: center">
  <tr>
    ...
    <td style="text-align: center; width: 30%">
      ...
      <br>
      <br>
      <?php echo $retur_data[0]->retur_by?> 
    </td>
  </tr>
...
<div id="options">
<button id="printpagebutton" style="font-family: arial; background: blue; color: white; cursor: pointer" onclick="printpage()" style="cursor: pointer"/>Print Nota Retur Farmasi</button>
</div>
