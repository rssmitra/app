...
class Ao_log_receipt extends MX_Controller {
    ...
    function __construct() 
    {
        ...
        parent::__construct();
        /*breadcrumb default*/
        $this->breadcrumbs->push('Index', 'farmasi/Ao_log_receipt');
        /*session redirect login if not login*/
        if($this->session->userdata('logged')!=TRUE){
            echo 'Session Expired !'; exit;
        }
        /*load model*/
        $this->load->model('Ao_log_receipt_model', 'Ao_log_receipt');
        /*enable profiler*/
    ...
    { 
        ...
        /*load view index*/
        $this->load->view('Ao_log_receipt/index', $data);
    }
    ...
    public function form($id='')
    {
        ...
        $this->breadcrumbs->push('Receipt '.strtolower($this->title).'', 'Ao_log_receipt/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
        /*get value by id*/
        $data['value'] = $this->Ao_log_receipt->get_by_id($id);
        ...
        /*load form view*/
        $this->load->view('Ao_log_receipt/form', $data);
    }

    public function get_data()
    {
        /*get data from model*/
        $list = $this->Ao_log_receipt->get_datatables();
        ...
        foreach ($list as $row_list) {
            $no++;
            $row = array();
            // $status = ($row_list->received_time == NULL) ? '<a href="#" class="btn btn-xs btn-primary" onclick="getMenu('."'farmasi/Ao_log_receipt/form/".$row_list->kode_trans_far."'".')"><i class="fa fa-check-square-o"></i></a>' : '<i class="fa fa-check-circle bigger-120 green"></i>';
            ...
            $row[] = '<div class="center">'.$row_list->persentase.'</div>';
            $data[] = $row;
        }

        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Ao_log_receipt->count_all(),
                        "recordsFiltered" => $this->Ao_log_receipt->count_filtered(),
                        "data" => $data,
        );
    ...
    public function process()
    {
        ...
        {                       
            ...
            $this->db->update('fr_pickup_obat', $dtpickup, array('pickup_id' => $_POST['kode']) );
    ...
    public function get_summary(){
        $list = $this->Ao_log_receipt->get_summary();
        foreach ($list as $key => $row_list) {
            ...
            if ($row_list->bagi_hasil != NULL) {
                $diambil[] = ($row_list->cost * 0.5);
            }else{
                $blm_diambil[] = ($row_list->cost * 0.5);
            }
        }
        $output = array(
                        ...
                        "bagi_hasil" => array_sum($received) * 0.5,
                        "diambil" => array_sum($diambil),
                        "blm_diambil" => array_sum($blm_diambil),
        );
        echo json_encode($output);
    }
