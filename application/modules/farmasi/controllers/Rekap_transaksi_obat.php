...
class Rekap_transaksi_obat extends MX_Controller {
    ...
    public function form($id='')
    {
        ...
        $this->breadcrumbs->push('Entry Resep '.strtolower($this->title).'', 'Rekap_transaksi_obat/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
        /*get value by id*/
        $data['value'] = $this->Rekap_transaksi_obat->get_by_id($id);
        $data['detail_obat'] = $this->Rekap_transaksi_obat->get_detail_resep_data($id)->result();
        ...
        /*load form view*/
        $this->load->view('Rekap_transaksi_obat/form', $data);
    }
    ...
    public function preview_etiket(){
        ...
        $resep_log = $this->Rekap_transaksi_obat->get_etiket_data();
        // echo '<pre>';print_r($resep_log->result());die;
        $data = array();
        $data['result'] = $resep_log->result();
        $this->load->view('farmasi/Rekap_transaksi_obat/preview_etiket', $data);
    ...
    public function preview_copy_resep($kode_trans_far){
        ...
        $this->load->view('farmasi/Rekap_transaksi_obat/preview_copy_resep', $data);

    }

    function nota_retur($no_retur){
        $his_retur = $this->Rekap_transaksi_obat->get_history_retur_by_no_retur($no_retur);
        $data = array(
            'retur_data' => $his_retur,
        );
        // echo '<pre>';print_r($data);die;
        $this->load->view('farmasi/Rekap_transaksi_obat/preview_nota_retur', $data);

    }

    public function print_tracer_obat($kode_trans_far)
    {   
        $resep_log = $this->Rekap_transaksi_obat->get_detail_resep_data($kode_trans_far);
        // echo '<pre>';print_r($resep_log->result());die;
        $this->print_escpos->print_resep_gudang($resep_log->result());
        $data = array();
        $data['result'] = $resep_log->result();
        $this->load->view('farmasi/Rekap_transaksi_obat/preview_tracer_obat', $data);
    }
    ...
    public function show_retur_data($id){
        ...
        $html_btn = '';
        if(count($data) > 0){
            $his_retur = $this->Rekap_transaksi_obat->get_history_retur($data[0]->kode_trans_far);
            ...
            if(count($his_retur) > 0){
                ...
                foreach ($his_retur as $khr => $vhr) {
                    ...
                    $html .= '&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" onclick="PopupCenter('."'farmasi/Rekap_transaksi_obat/nota_retur/".$khr."'".')"><i class="fa fa-print dark bigger-150"></i> </a>';
