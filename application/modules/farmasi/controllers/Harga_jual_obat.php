...
class Harga_jual_obat extends MX_Controller {
    ...
    public function process()
    {
        ...
        {                       
            $this->db->trans_begin();
            $id = ( count($this->Harga_jual_obat->get_by_id( $_POST['id'] )) > 0 ) ? $this->regex->_genRegex($this->input->post('id'),'RGXQSL'):'0';
            ...
            if(isset($_FILES['path_image'] ['name']) AND $_FILES['path_image'] ['name'] != ''){
                ...
                if( $id != 0 ){
                    $res_dt = $this->Harga_jual_obat->get_by_id($id);
            ...
            if( $id == '0' ){
                ...
                /*update record*/
                // print_r($dataexc);die;
                $this->Harga_jual_obat->update($table, array('kode_brg' => $id), $dataexc);
                $newId = $id;
                /*save logs*/
    ...
    public function delete()
    {
        ...
        $toArray = explode(',',$id);
        if($id!=null){
            if($this->Harga_jual_obat->delete_by_id($toArray)){
                // $this->logs->save('Harga_jual_obat', $id, 'delete record', '', 'kode_brg');
    ...
    public function print_multiple_preview(){
        ...
        $result = $this->Harga_jual_obat->get_by_id($_GET['id']);
        // print_r($result);die;
        $table = ($_GET['flag']=='non_medis')?'mt_barang_nm':'mt_barang';
        ...
        if(isset($_GET['tipe']) AND $_GET['tipe']=='kartu_stok'){
            $this->load->view('farmasi/Harga_jual_obat/print_label_kartu_stok', $data);
        }else{
            $this->load->view('farmasi/Harga_jual_obat/print_label', $data);
        }
    }
