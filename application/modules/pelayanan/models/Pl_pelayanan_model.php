...
class Pl_pelayanan_model extends CI_Model {
	...
	function get_data_antrian_pasien()
	{
		...
		$this->db->select('CONVERT (varchar(10), tgl_jam_poli, 103) as format_tgl_poli');
		$this->_main_query();
		...
		if (isset($_GET['status']) && $_GET['status'] != '') {
			if($_GET['status'] == 2){
				...
				$this->db->where('pl_tc_poli.tgl_keluar_poli is null');
			}
        }
	...
	{
		$this->db->select('nama_pasien, almt_ttp_pasien, umur, tc_registrasi.no_mr, jen_kelamin, tgl_lhr, tempat_lahir');
