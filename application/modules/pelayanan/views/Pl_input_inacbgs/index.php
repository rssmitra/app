...
$(document).ready(function(){
  ...
  oTable = $('#dynamic-table').DataTable({ 
    ...
    /*"ajax": {
        "url": "pelayanan/Pl_input_inacbgs/get_data?search_by="+$("#search_by").val()+"&keyword="+$("#keyword_form").val()+"&from_tgl="+$("#from_tgl").val()+"&to_tgl="+$("#to_tgl").val()+"",
        "type": "POST"
    },*/
     "ajax": {
        "url": "pelayanan/Pl_input_inacbgs/get_data?search_by="+$("#search_by").val()+"&keyword="+$("#keyword_form").val()+"&is_icu="+$("#is_icu").val()+"",
        "type": "POST"
    },
    "columnDefs": [
        { 
  ...
  $('#dynamic-table tbody').on('click', 'td.details-control', function () {
    ...
    $('#btn_search_data').click(function (e) {
        e.preventDefault();
        $.ajax({
        url: 'pelayanan/Pl_input_inacbgs/find_data',
        type: "post",
        ...
        success: function(data) {
          achtungHideLoader();
          find_data_reload(data,'pelayanan/Pl_input_inacbgs');
        }
      });
    });
...
function find_data_reload(result=''){
  ...
  oTable.ajax.url('pelayanan/Pl_input_inacbgs/get_data?'+result.data).load();
  $("html, body").animate({ scrollTop: "400px" });

}


function udpateRow(no_registrasi){
  
  var inacbgs = $('#inacbgs_'+no_registrasi+'').val();
  var tarif = $('#tarif_inacbgs_'+no_registrasi+'').val();
  $.ajax({
      url: "pelayanan/Pl_input_inacbgs/process",
      data: {no_registrasi : no_registrasi, ina_cbgs : inacbgs, tarif_inacbgs : tarif },
      dataType: "json",
      type: "POST",
      complete: function (xhr) {
        ...
        if(jsonResponse.status === 200){  
          ...
          /*reload table*/
          oTable.ajax.url('pelayanan/Pl_input_inacbgs/get_data').load();
        }else{          
          /*$.achtung({message: jsonResponse.message, timeout:5});  */
        } 
        /*achtungHideLoader();*/
...
<div class="row">
  <div class="col-xs-12">
    ...
    <form class="form-horizontal" method="post" id="form_search" action="pelayanan/Pl_input_inacbgs/find_data">
    ...
    <div style="margin-top:-27px">
      <table id="dynamic-table" base-url="pelayanan/Pl_input_inacbgs" class="table table-bordered table-hover">
       <thead>
        <tr>  
          <th width="50px">&nbsp;</th>
          <th width="50px">&nbsp;</th>
          ...
          <th>Dokter</th>
          <th>Status</th>          
          <th width="150px">Diagnosa Inacbgs</th>
          <th class="center" width="100px"> Tarif inaCBG</th>
        </tr>
      </thead>
      <tbody>
      </tbody>
