...
class Penerimaan_aset extends MX_Controller {
    ...
    function __construct() {
        ...
        parent::__construct();
        /*breadcrumb default*/
        $this->breadcrumbs->push('Index', 'inventory/aset/Penerimaan_aset');
        /*session redirect login if not login*/
        if($this->session->userdata('logged')!=TRUE){
            echo 'Session Expired !'; exit;
        }
        /*load model*/
        $this->load->model('inventory/aset/Penerimaan_aset_model', 'Penerimaan_aset');
        // load library
    ...
    public function index() { 
        ...
        /*load view index*/
        $this->load->view('inventory/aset/Penerimaan_aset/index', $data);
    }

    public function get_detail($id){

        $result = $this->Penerimaan_aset->get_detail_table($id);
        ...
        $temp_view = $this->load->view('inventory/aset/Penerimaan_aset/detail_table_view', $data, true);
    ...
    public function form($id='')
    {
        ...
        $this->load->view('inventory/aset/Penerimaan_aset/form', $data, false);
    ...
    {
        /*get data from model*/
        $list = $this->Penerimaan_aset->get_datatables();
        ...
        foreach ($list as $row_list) {
            ...
            $row[] = '<div class="left">['.$row_list->kode_brg.'] '.$row_list->nama_brg.'</div>';
            $row[] = '<div class="left">['.$row_list->kode_sub_gol.'] '.$row_list->nama_sub_golongan.'</div>';
            $row[] = '<div class="center">'.$row_list->jumlah_besar.'</div>';
            $row[] = '<div class="center">'.$row_list->satuan_besar.'</div>';
            $total = $row_list->jumlah_besar * $row_list->harga_satuan;
            $row[] = '<div class="right" style="text-align: right">'.number_format($row_list->harga_satuan).'</div>';
            $row[] = '<div class="right" style="text-align: right">'.number_format($total).'</div>';
            $row[] = '<div class="center">
                        <a href="#" onclick="PopupCenter('."'purchasing/po/Po_penerbitan/print_preview?ID=".$row_list->id_tc_po."&flag=non_medis'".','."'Cetak'".',900,650);" class="btn btn-xs btn-default" title="cetak po"><i class="fa fa-print dark"></i></a>
                      </div>';
            $row[] = '<div class="center">
                      <a href="#" onclick="getMenu('."'inventory/aset/Penerimaan_aset/form/".$row_list->id_tc_po."?flag=non_medis'".')" class="btn btn-xs btn-primary" title="revisi po">Terima </a>
                    </div>';
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Penerimaan_aset->count_all(),
                        "recordsFiltered" => $this->Penerimaan_aset->count_filtered(),
                        "data" => $data,
                );
        ...
        {                       
            ...
            if( $_POST['submit'] == 'header' ){
                ...
                if($id==0){
                    ...
                    $last_id = $this->Penerimaan_aset->save($table, $dataexc);
                    ...
                    // print_r($dataexc);die;
                    /*update record*/
                    $this->Penerimaan_aset->update($table, array('id_penerimaan' => $id), $dataexc);
                    $newId = $id;
            ...
            if( $_POST['submit'] == 'penerimaan_selesai' ){
                ...
                foreach ($_POST['is_checked'] as $keys => $rows) {
                    ...
                    $po_dt = $this->Penerimaan_aset->get_sisa_penerimaan($tc_po.'_det', $table.'_detail', $_POST['id_tc_po']);
        ...
        {                       
            ...
            if( $_POST['submit'] == 'input_batch' ){
                ...
                if($id==0){
                    $dataexc['created_date'] = date('Y-m-d H:i:s');
                    $dataexc['created_by'] = json_encode(array('user_id' =>$this->regex->_genRegex($this->session->userdata('user')->user_id,'RGXINT'), 'fullname' => $this->regex->_genRegex($this->session->userdata('user')->fullname,'RGXQSL')));
                    $this->Penerimaan_aset->save('tc_penerimaan_brg_batch_log', $dataexc);
                    ...
                    /*print_r($dataexc);die;*/
                    /*update record*/
                    $this->Penerimaan_aset->update('tc_penerimaan_brg_batch_log', array('id_tc_batch_log' => $id), $dataexc);
                    $newId = $id;
    ...
    public function preview_penerimaan(){
        ...
        $this->load->view('inventory/aset/Penerimaan_aset/preview_penerimaan_brg', $data);
    }
