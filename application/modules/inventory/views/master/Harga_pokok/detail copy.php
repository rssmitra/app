...
<div class="table-responsive">
    <table class="hp-table-detail">
        ...
        <?php foreach ($po_hist as $i => $po) :
            ...
            $harga_setelah_diskon = ($po->discount > 0) ? (float)$harga_sebelum_diskon * (1 - (float)$po->discount / 100) : (float)$po->harga_satuan;
            ...
            if ($i === 0) {
                ...
                $prev_po    = $po_hist[$i - 1];
                $prev_stlh_diskon = ($prev_po->discount > 0)
                    ? (float)$prev_po->harga_satuan * (1 - (float)$prev_po->discount / 100)
                    : (float)$prev_po->harga_satuan;
                $prev_netto = $prev_stlh_diskon * (1 + (float)$prev_po->ppn / 100);
...
<?php if (count($po_hist) > 1) :
    ...
    $po_baru = $po_hist[0];
    $oldest_stlh_diskon = ($po_tua->discount > 0)
        ? (float)$po_tua->harga_satuan * (1 - (float)$po_tua->discount / 100)
        : (float)$po_tua->harga_satuan;
    $oldest  = $oldest_stlh_diskon * (1 + (float)$po_tua->ppn / 100);
    $newest_stlh_diskon = ($po_baru->discount > 0)
        ? (float)$po_baru->harga_satuan * (1 - (float)$po_baru->discount / 100)
        : (float)$po_baru->harga_satuan;
    $newest  = $newest_stlh_diskon * (1 + (float)$po_baru->ppn / 100);
...
<?php
// ── Weighted Average Harga Pokok ──
$wa_sum_qty   = 0;
$wa_sum_value = 0;
foreach ($po_hist as $po) {
    $wa_stlh_diskon = ($po->discount > 0)
        ? (float)$po->harga_satuan * (1 - (float)$po->discount / 100)
        : (float)$po->harga_satuan;
    $wa_h_netto     = $wa_stlh_diskon * (1 + (float)$po->ppn / 100);
    $wa_qty         = (float)$po->jumlah_besar;
    $wa_sum_qty    += $wa_qty;
    $wa_sum_value  += $wa_qty * $wa_h_netto;
}
$wa_harga_besar = ($wa_sum_qty > 0) ? $wa_sum_value / $wa_sum_qty : 0;
$wa_harga_kecil = ($rasio > 0 && $wa_harga_besar > 0) ? $wa_harga_besar / $rasio : $wa_harga_besar;
$wa_harga_jual  = $wa_harga_kecil * (4 / 3) * 1.11;
?>
...
<div style="margin-top:14px;border:2px solid #0891b2;border-radius:8px;padding:12px 16px;background:#f0f9ff">
    ...
    <table style="width:100%;font-size:12px;border-collapse:collapse">
        <tr>
            <td style="padding:4px 8px;color:#475569;width:55%">Harga Pokok WA per Satuan Besar</td>
            <td style="padding:4px 8px;font-weight:700;color:#0f172a;text-align:right">
                Rp <?php echo number_format($wa_harga_besar, 2, ',', '.') ?>
            </td>
        </tr>
        <tr style="background:#e0f2fe;border-radius:4px">
            <td style="padding:5px 8px;color:#0369a1;font-weight:700">
                Harga Pokok WA per Satuan Kecil
            ...
            <td style="padding:5px 8px;font-weight:800;color:#0369a1;font-size:14px;text-align:right">
                Rp <?php echo number_format($wa_harga_kecil, 2, ',', '.') ?>
            </td>
        </tr>
        <tr>
            <td colspan="2"><hr style="margin:8px 0;border-color:#bae6fd"></td>
        </tr>
        <tr>
            <td style="padding:4px 8px;color:#475569">
                Estimasi Harga Jual
                <small style="color:#94a3b8">(WA &times; Margin 33,3% &times; PPN 11%)</small>
            </td>
            <td style="padding:4px 8px;text-align:right">
                <span style="background:#0891b2;color:#fff;border-radius:4px;padding:3px 10px;font-weight:800;font-size:13px">
                    Rp <?php echo number_format($wa_harga_jual, 0, ',', '.') ?>
                </span>
            </td>
        </tr>
    </table>
    <div style="margin-top:8px;font-size:10px;color:#94a3b8">
        * Berdasarkan bobot quantity setiap PO &bull;
        Rumus: &Sigma;(Qty &times; Harga Netto) / &Sigma;Qty
    </div>
</div>
    ...
    <table class="hp-table-detail">
        <thead>
            <tr>
                ...
                <th rowspan="2" class="text-right" width="250px">Harga Satuan (AVG)</th>
                <th class="text-right" style="background:#fef9c3; width: 250px">(AVG) Harga Jual Satuan<br>(Margin + PPN)</th>
            </tr>
        </thead>
        <tbody>
        <?php $prev_harga = null; foreach ($sales_hist as $j => $tr) :
            $hs_avg    = (float)$tr->harga_satuan_avg;
            $h_jual    = (float)$tr->harga_satuan;
            $total_qty = (int)$tr->total_qty;
            $total_ppn = (int)round($total_qty * $hs_avg * (4 / 3) * 0.11);

            if ($prev_harga !== null) {
                if ($h_jual < $prev_harga)      $arrow = '<i class="fa fa-arrow-down hp-trend-down" title="Turun"></i>';
                elseif ($h_jual > $prev_harga)  $arrow = '<i class="fa fa-arrow-up hp-trend-up"   title="Naik"></i>';
        ...
        ?>
            <tr>
                ...
                <td class="text-right" style="background:#fefce8">
                    <strong>Rp <?php echo number_format($h_jual, 0, ',', '.') ?></strong>
                    <?php echo $arrow ?>
                </td>
            </tr>
        <?php endforeach ?>
...
<?php
$harga_satuan_ref = (count($sales_hist) > 0 && isset($sales_hist[0]->harga_satuan_avg))
    ? (float)$sales_hist[0]->harga_satuan_avg
    : 0;
    ...
    <table class="hp-table-detail">
        <thead>
            <tr>
                ...
                <th>Satuan</th>
                <th class="text-right">Harga Satuan (Ref. Trx Terakhir)</th>
            ...
            <tr>
                ...
                <td class="text-right">
                    <?php echo $harga_satuan_ref > 0
                        ? 'Rp ' . number_format($harga_satuan_ref, 0, ',', '.')
