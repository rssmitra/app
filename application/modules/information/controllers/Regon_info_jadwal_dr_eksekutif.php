...
class Regon_info_jadwal_dr_eksekutif extends MX_Controller {
    ...
    function __construct() {
        ...
        parent::__construct();
        /*breadcrumb default*/
        $this->breadcrumbs->push('Index', 'information/Regon_info_jadwal_dr_eksekutif');
        /*session redirect login if not login*/
        if($this->session->userdata('logged')!=TRUE){
            echo 'Session Expired !'; exit;
        }
        /*load model*/
        $this->load->model('Regon_info_jadwal_dr_eksekutif_model', 'Regon_info_jadwal_dr_eksekutif');
    ...
    public function index() { 
        ...
        /*load view index*/
        $this->load->view('Regon_info_jadwal_dr_eksekutif/index', $data);
    }

    public function lihat_jadwal_dokter() { 
        
        $this->load->view('Regon_info_jadwal_dr_eksekutif/tab_lihat_dokter');
    
    }

    public function cuti_dokter() { 
        
        $this->load->view('Regon_info_jadwal_dr_eksekutif/tab_cuti_dokter');
    
    }

    public function jadwal_dokter() { 
        
        $this->load->view('Regon_info_jadwal_dr_eksekutif/tab_jadwal_dokter');
    ...
    {
        /*get data from model*/
        $list = $this->Regon_info_jadwal_dr_eksekutif->get_datatables();
        $arrData = array();
        /*format data*/
        ...
        foreach ($arrData as $key => $row_list) {
            ...
            foreach ($row_list as $key_2 => $value_2) {
                ...
                $row[] = '<div class="center">
                            '.$this->authuser->show_button('information/Regon_info_jadwal_dr_eksekutif','U',$main_data['jd_id'],2).'
                            '.$this->authuser->show_button('information/Regon_info_jadwal_dr_eksekutif','D',$main_data['jd_id'],7).'
                          </div>'; 
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Regon_info_jadwal_dr_eksekutif->count_all(),
                        "recordsFiltered" => $this->Regon_info_jadwal_dr_eksekutif->count_filtered(),
                        "data" => $data,
                );
    ...
    public function form($id='')
    {
        ...
        if( $id != '' ){
            ...
            $this->breadcrumbs->push('Edit '.strtolower($this->title).'', 'Regon_info_jadwal_dr_eksekutif/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
            /*get value by id*/
            $data['value'] = $this->Regon_info_jadwal_dr_eksekutif->get_by_id($id);
            $data['jadwal'] = $this->Regon_info_jadwal_dr_eksekutif->get_jadwal_by_dr_spesialis($data['value']);
            /*initialize flag for form*/
            $data['flag'] = "update";
            $data['id'] = $id;
        }else{
            /*breadcrumbs for create or add row*/
            $this->breadcrumbs->push('Add '.strtolower($this->title).'', 'Regon_info_jadwal_dr_eksekutif/'.strtolower(get_class($this)).'/form');
        ...
        /*load form view*/
        $this->load->view('Regon_info_jadwal_dr_eksekutif/form', $data);
    }
    /*function for view data only*/
    public function show($id)
    {
        /*breadcrumbs for view*/
        $this->breadcrumbs->push('View '.strtolower($this->title).'', 'Regon_info_jadwal_dr_eksekutif/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
    ...
    public function process()
    {
        ...
        {                       
            ...
            if( $this->input->post('flag')=='create' ){
                $is_exist = $this->Regon_info_jadwal_dr_eksekutif->is_exist( array('jd_kode_dokter' => $val->set_value('dokter'), 'jd_kode_spesialis' => $val->set_value('spesialis') ) );
            ...
            foreach ($jd_hari as $key => $value) {
                ...
                if($delete[$value] != ''){
                    ...
                    $data_sms_regon = $this->Regon_info_jadwal_dr_eksekutif->get_no_booking($val->set_value('dokter'),$val->set_value('spesialis'),$jd_hari[$key]);

                    //print_r($this->db->last_query());                    
                    $days = $this->tanggal->getHariTranslate($jd_hari[$key]);

                    $data_sms_pesanan = $this->Regon_info_jadwal_dr_eksekutif->get_no_pesanan($val->set_value('dokter'),$val->set_value('spesialis'),$days);
                    ...
                    if($start[$key] != ''){
                        ...
                        if($jd_id[$key] != 0){
                            ...
                            if($curr_edit[$key]!=''){
                                ...
                                /*update record*/
                                $this->Regon_info_jadwal_dr_eksekutif->update(array('jd_id' => $jd_id[$key]), $dataexc);
                                $newId = $jd_id[$key];
                                /*save logs*/
                            ...
                            //print_r($dataexc);die;
                            /*save post data*/
                            $newId = $this->Regon_info_jadwal_dr_eksekutif->save($dataexc);
                            /*save logs*/
    ...
    public function delete()
    {
        ...
        $toArray = explode(',',$id);
        if($id!=null){
            if($this->Regon_info_jadwal_dr_eksekutif->delete_by_id($toArray)){
