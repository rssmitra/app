...
class Eks_poli_model extends CI_Model {
    ...
    private function _main_query(){
        ...
        // rawat jalan
        if($_GET['jenis_kunjungan'] == 'rj'){
            // poli rawat jalan
            $poli = $this->getRekapPoli();
            echo "<pre>"; print_r($_GET); die;
            // IGD
            $igd = $this->getRekapIgd();
            // penunjang medis
            $pm = $this->getRekapPm();
        }else if($_GET['jenis_kunjungan'] == 'ri'){
            // ri tc rawat inap
            $ri = $this->getRekapRI();
        }else{
    ...
    private function _qrygetRekapPoli(){

        $this->db->select('COUNT(*) as ttl_pasien, CAST((SUM(bill_rs) + SUM(bill_dr1) + SUM(bill_dr2) + SUM(bill_dr3)) as INT) as total, d.nama_bagian');
        $this->db->from('pl_tc_poli a');
    ...
    public function getRekapPoli(){
        // by periode
        $this->_qrygetRekapPoli();
        $this->db->where('CAST(tgl_jam_poli as DATE) BETWEEN '."'".$_GET['from_tgl']."'".' AND '."'".$_GET['to_tgl']."'".' ');
        $this->db->where('a.status_batal is null');
        $this->db->group_by('d.nama_bagian');
    ...
    public function getRekapIgd(){
        $this->db->select('CAST((SUM(bill_rs) + SUM(bill_dr1) + SUM(bill_dr2) + SUM(bill_dr3)) as INT) as total');
        $this->db->join('tc_trans_pelayanan b','b.no_kunjungan=a.no_kunjungan');
        if (isset($_GET['penjamin']) AND $_GET['penjamin'] != 'all') {  
            if (isset($_GET['penjamin']) AND $_GET['penjamin'] == 'bpjs') { 
                $this->db->where('b.kode_perusahaan', 120);
            }
        ...
        $igd = $this->db->get_where('gd_tc_gawat_darurat a');
        return $igd;
    }

    public function getRekapPm(){
        $this->db->select('CAST((SUM(bill_rs) + SUM(bill_dr1) + SUM(bill_dr2) + SUM(bill_dr3)) as INT) as total');
        $this->db->join('tc_trans_pelayanan b','b.no_kunjungan=a.no_kunjungan');
        if (isset($_GET['penjamin']) AND $_GET['penjamin'] != 'all') {  
            if (isset($_GET['penjamin']) AND $_GET['penjamin'] == 'bpjs') { 
        ...
        $pm = $this->db->get_where('pm_tc_penunjang a');
        return $pm;
    }

    public function getRekapRI(){
        $this->db->select('CAST((SUM(bill_rs) + SUM(bill_dr1) + SUM(bill_dr2) + SUM(bill_dr3)) as INT) as total');
        $this->db->join('tc_trans_pelayanan b','b.no_kunjungan=a.no_kunjungan');
        if (isset($_GET['penjamin']) AND $_GET['penjamin'] != 'all') {  
            if (isset($_GET['penjamin']) AND $_GET['penjamin'] == 'bpjs') { 
        ...
        $pm = $this->db->get_where('ri_tc_rawatinap a');
        return $pm;
    }
