...
<div class="row">
    <div class="col-xs-12">
        <form class="form-horizontal" method="post" id="form_booking" action="<?php echo site_url('registration/Reg_pasien/process_perjanjian')?>" enctype="multipart/form-data" autocomplete="off"> 
                    ...
                    <tbody>
                        <tr>
                            <td  style="text-align: left !important">
                                <span style="font-size: 14px; font-weight: bold"><?php echo $value->nama_pegawai?></span><br>
                                ...
                                <td>
                                    <?php 
                                        if(isset($jadwal_dr_spesialis[$day])) :
                                            echo '<a href="#" onclick="changeDay('.$jadwal_dr_spesialis[$day]['jd_id'].', '."'".$jadwal_dr_spesialis[$day]['jd_hari']."'".', '."'".$jadwal_dr_spesialis[$day]['jd_jam_mulai']."'".')" style="font-size:14px; font-weight: bold">'.$this->tanggal->formatTime($jadwal_dr_spesialis[$day]['jd_jam_mulai']).' s.d '. $this->tanggal->formatTime($jadwal_dr_spesialis[$day]['jd_jam_selesai']).'</a><br>Kuota : '.$jadwal_dr_spesialis[$day]['jd_kuota'].'<br><small>'.$jadwal_dr_spesialis[$day]['jd_keterangan'].'</small>';
                                        else:
                                            echo '-';
                                        endif; 
                                    ?>
                            ...
                            <!-- <td style="vertical-align: middle; text-align: center">
                                <a href="#" class="btn btn-sm btn-danger" style="font-size: 14px;font-weight: bold;" onclick="scrollSmooth('Self_service/jadwal_dokter?kode=<?php echo trim($value->jd_kode_spesialis)?>')"><i class="fa fa-refresh"></i></a>
                            </td> -->
            ...
            <div class="row" id="resultSearchPasien">
                ...
                <div id="tgl_kunjungan_form">
                    ...
                    <div class="col-sm-4 padding-10">
                        ...
                        <div class="center">
                            ...
                            <div style="text-align: center !important; padding-left: 73px !important">
                                <span style="text-align: left !important; font-weight: bold;font-size: 14px;width: 100% !important; margin-left: -17%">PILIH TANGGAL UNTUK HARI <span id="txt_day"><?php echo strtoupper($value->jd_hari)?></span></span>
                                <div id="datepicker_inline"></div>
                            </div>

                            <button type="button" class="btn btn-block" style="background : green !important; border-color: green; height: 45px !important; width: 300px;height: 40px !important; margin-left:4px; font-size: 14px; font-weight: bold" onClick="showTestDate()"><i class="fa fa-calendar"></i> PILIH TANGGAL</button>
                            
                            <input name="tanggal_kunjungan" id="tgl_kunjungan_input" value="" placeholder="<?php echo $this->tanggal->formatDateForm(date('Y-m-d'))?>" type="hidden">
...
function showTestDate(){ 
    ...
    $.post('<?php echo site_url('Templates/References/CheckSelectedDate') ?>', {date:selected_date, kode_spesialis:spesialis, kode_dokter:dokter, jadwal_id:jd_id} , function(data) {
        ...
        if(data.status=='expired'){
            ...
            var message = '<div class="alert alert-block center"></p></div>';
        ...
        //   achtungHideLoader();
          if(response.status==200){
            ...
            if(response.count_perjanjian > 0){
                ...
                html_p += '<div style="text-align: left !important; font-weight: bold;font-size: 14px; ">DATA PERJANJIAN PASIEN TERAKHIR</div>';
                html_p += '<table class="table">';
                html_p += '<tr style="background: #87b87f;font-size: 14px;font-weight: bold">';
                html_p += '<td>Kode Booking</td>';
                html_p += '<td>Nama Dokter</td>';
                html_p += '<td>Tanggal Dibuat</td>';
                html_p += '<td width="280px">#</td>';
                html_p += '</tr>';
                html_p += '<tr style="font-size: 14px;">';
            ...
            html += '<div style="text-align: left !important; font-weight: bold;font-size: 14px; ">DATA PASIEN</div>';
            // html += '<table class="table">';
            // html += '<tr style="background: #f9eb41;font-size: 14px;font-weight: bold">';
            // html += '<td>No MR</td>';
            // html += '<td>No KTP</td>';
