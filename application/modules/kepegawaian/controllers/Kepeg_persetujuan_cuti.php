...
class Kepeg_persetujuan_cuti extends MX_Controller {
    ...
    function __construct() {
        ...
        parent::__construct();
        /*breadcrumb default*/
        $this->breadcrumbs->push('Index', 'kepegawaian/Kepeg_persetujuan_cuti');
        /*session redirect login if not login*/
        if($this->session->userdata('logged')!=TRUE){
            echo 'Session Expired !'; exit;
        }
        /*load model*/
        $this->load->model('Kepeg_persetujuan_cuti_model', 'Kepeg_persetujuan_cuti');
        /*enable profiler*/
    ...
    public function index() { 
        ...
        /*load view index*/
        $this->load->view('Kepeg_persetujuan_cuti/index', $data);
    }
    ...
    public function form($id='')
    {
        ...
        $this->breadcrumbs->push('Edit '.strtolower($this->title).'', 'Kepeg_persetujuan_cuti/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
        /*get value by id*/
        $data['value'] = $this->Kepeg_persetujuan_cuti->get_by_id($id);
        ...
        /*load form view*/
        $this->load->view('Kepeg_persetujuan_cuti/form', $data);
    }
    ...
    public function show($id)
    {
        ...
        $this->breadcrumbs->push('View '.strtolower($this->title).'', 'Kepeg_persetujuan_cuti/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
    ...
    {   
        ...
        // print_r($fields);die;
        $data = $this->Kepeg_persetujuan_cuti->get_by_id($id);
    ...
    public function get_data()
    {
        ...
        foreach ($list as $row_list) {
            ...
            $row[] = $status;
            $row[] = '<div class="center">
                        <a href="#" style="width: 100% !important" class="label label-xs label-success" onclick="getMenu('."'kepegawaian/Kepeg_persetujuan_cuti/form/".$row_list->pengajuan_cuti_id."'".')"><i class="fa fa-pencil"></i> Persetujuan</a>
                    </div>';
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Kepeg_persetujuan_cuti->count_all(),
                        "recordsFiltered" => $this->Kepeg_persetujuan_cuti->count_filtered(),
                        "data" => $data,
                );
        ...
        {                       
            ...
            $dt_pegawai = $this->db->get_where('view_dt_pegawai', array('kepeg_id' => $val->set_value('acc_by_kepeg_id') ) )->row();
             if(!empty($dt_pegawai)){
                ...
                // log acc
                $spv = $this->Kepeg_persetujuan_cuti->get_spv($dt_pegawai->kepeg_unit, $acc_level);
                if(!empty($spv)){
                    $dataacc = array(
                    ...
                    $this->Kepeg_persetujuan_cuti->save('kepeg_log_acc_pengajuan', $dataacc);
                }
            }
    ...
    public function delete()
    {
        ...
        $toArray = explode(',',$id);
        if($id!=null){
            if($this->Kepeg_persetujuan_cuti->delete_by_id($toArray)){
