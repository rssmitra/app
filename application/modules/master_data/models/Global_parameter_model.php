...
class Global_parameter_model extends CI_Model {
	...
	var $order = array('global_parameter.auto_id' => 'DESC');
	...
	public function get_by_id($id)
	{
		$this->_main_query();
		if(is_array($id)){
			$this->db->where_in(''.$this->table.'.auto_id',$id);
			$query = $this->db->get();
			return $query->result();
		}else{
			$this->db->where(''.$this->table.'.auto_id',$id);
			$query = $this->db->get();
			return $query->row();
		}
	...
	{
		$this->db->where_in(''.'global_parameter'.'.auto_id', $id);
