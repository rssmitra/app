...
$(document).ready(function() {
  //initiate dataTables plugin
    oTable = $('#dynamic-table').DataTable({ 
      ...
      "ajax": {
          "url": "casemix/Csm_reg_not_merge/get_data",
          "type": "POST"
      },
      /*"columnDefs": [
        { 
    ...
    $('#dynamic-table tbody').on('click', 'td.details-control', function () {
            ...
            if ( row.child.isShown() ) {
                ...
                $.getJSON("casemix/Csm_reg_not_merge/getDetail/" + inv, '', function (data) {
    ...
    $('#dynamic-table tbody').on( 'click', 'tr', function () {
      ...
      $('#btn_search_data').click(function (e) {
          e.preventDefault();
          $.ajax({
          url: 'casemix/Csm_reg_not_merge/find_data',
          type: "post",
      ...
      $('#btn_export_excel').click(function (e) {
          ...
          window.open('Templates/export_data/exportContent?month='+month+'&year='+year+'&tipe='+tipe+'&frmdt='+from_tgl_reg+'&todt='+to_tgl_reg+'&field='+field+'&type=excel&mod=Csm_reg_not_merge','_blank');     
      });
      ...
      $("#button_submit_multiple").click(function(event){
          event.preventDefault();
          var searchIDs = $("#dynamic-table input:checkbox:checked").map(function(){
            return $(this).val();
          }).toArray();
          submit_multiple_action(''+searchIDs+'')
          console.log(searchIDs);
      });
...
function reset_table(){
    oTable.ajax.url('casemix/Csm_reg_not_merge/get_data').load();
...
function find_data_reload(result){
    ...
    oTable.ajax.url('casemix/Csm_reg_not_merge/get_data?month='+data.month+'&year='+data.year+'&tipe='+data.tipe+'&frmdt='+data.from_tgl_reg+'&todt='+data.to_tgl_reg+'&field='+data.search_by_field+'&search=excecute').load();
    $("html, body").animate({ scrollTop: "100px" });
...
function delete_data(myid){
  if(confirm('Are you sure?')){
    $.ajax({
        url: 'casemix/Csm_reg_not_merge/delete',
        type: "post",
        data: {ID:myid},
        dataType: "json",
        beforeSend: function() {
...
function submit_multiple_action(myid){
  if(confirm('Are you sure?')){
    $.ajax({
        url: 'casemix/Csm_reg_not_merge/submit_multiple',
        type: "post",
        data: {ID:myid},
        dataType: "json",
        beforeSend: function() {
        ...
        complete: function(xhr) {     
          ...
          if(jsonResponse.status === 200){
            $.achtung({message: jsonResponse.message, timeout:5});
            /*alert(jsonResponse.redirect_data.length);return false;*/
            for(var i = 0 ; i < jsonResponse.redirect_data.length ; i ++){
                 var cell = jsonResponse.redirect_data[i];
...
function submit(no_registrasi){
    ...
    $.ajax({
        ...
        complete: function(xhr) {     
          ...
          var jsonResponse = JSON.parse(data);
          if(jsonResponse.status === 200){
            window.open(jsonResponse.redirect, 'blank');
