    ...
    if($('#counter_number_value').val()!=0){
      ...
      $.ajax({
          ...
          success:function (data) {
            ...
            // var main = new LocketAudio();
            // main.play_Audio(data.loket, data.no, data.type);
        ...
        if( type != '#' ){
          ...
          $.post("antrian/Loket/reload_page", { loket:loket, tipe:type } ).done( function(data) {
            ...
            if(obj.success==1){
              ...
              $('#total_online').text(obj.total_online);
              $('#sisa_antrian_online').text(obj.sisa_online);
  ...
  function LocketAudio() {
    ...
    this.play_Audio = function(loket, no, type) {
      let self = this;
      self.queue.push({
        loket: loket,
        no: no,
        type : type
      }); // antrian di push ke array
    ...
    this._call = function() {
      let self = this;
      // disini dicek apakah antrian di array masih ada
      if (self.queue.length) {
        setTimeout(() => {
          
          playAudio(self.queue[0].no,self.queue[0].loket,self.queue[0].type);

          self.queue.shift(); // setelah dipanggil, index 0 diremove
          self._call(); // recursive / pengulangan
        }, Math.floor(Math.random() * 3000) + 1000); // random timeout antara 1 - 3 detik
      }
    }
